...
# @st.cache_data
def get_options_data(iterations=st.session_state.slider_value, risk_free=st.session_state.risk_free_rate):
    ...
    frontier.compute()
    st.session_state["data"] = portfolio, frontier
...
st.sidebar.write("This project is an implementation of the Markowitz efficient frontier model wich is part the modern portfolio theory (MPT). This Beta release will receive further updates.")
...
with overview_tab:
    with st.expander("Asset allocation", expanded=True):
        overview_tab4, overview_tab5, overview_tab6, overview_tab7, overview_tab8, overview_tab9 = st.tabs(["Max return (lo)", "Min volatility (lo)", "Max Sharpe (lo)", "Max return (op)", "Min volatility (op)", "Max Sharpe (op)"])
        
        with overview_tab4:
            st.info('The asset weights in the portfolio that maximize returns are located in the dataframe generated by the Monte Carlo simulation.', icon="ℹ️")
        ...
        with overview_tab5:
            st.info('The asset weights in the portfolio that minimise volatility are located in the dataframe generated by the Monte Carlo simulation.', icon="ℹ️")
            tab4_col1, tab4_col2 = st.columns((1,2))
            with tab4_col1:
        ...
        with overview_tab6:
            st.info('The asset weights in the portfolio that maximise the Sharpe ratio (Rp-Rf/σp) are located in the dataframe generated by the Monte Carlo simulation.', icon="ℹ️")
        ...
        for i in range(len(st.session_state.data[1].optimized_min_vol_port[2])):
            optimized_max_return_port[f"{st.session_state.data[0].tickers[i].upper()} weight"] = st.session_state.data[1].optimized_min_vol_port[2][i]
        ...
        for i in range(len(st.session_state.data[1].optimized_min_vol_port[2])):
            optimized_min_vol_port[f"{st.session_state.data[0].tickers[i].upper()} weight"] = st.session_state.data[1].optimized_min_vol_port[2][i]
        ...
        for i in range(len(st.session_state.data[1].optimized_min_vol_port[2])):
            optimized_optimal_risky_port[f"{st.session_state.data[0].tickers[i].upper()} weight"] = st.session_state.data[1].optimized_optimal_risky_port[2][i]

        with overview_tab7:
            st.info('The asset weights in the portfolio that maximise the returns are computed through an optimization algorithm', icon="ℹ️")
        ...
        with overview_tab8:
            st.info('The asset weights in the portfolio that minimize the volatility are computed through an optimization algorithm', icon="ℹ️")
        ...
        with overview_tab9:
            st.info('The asset weights in the portfolio that maximise the Sharpe ratio (Rp-Rf/σp) are computed through an optimization algorithm', icon="ℹ️")
    ...
    with st.expander("Portfolio optimization", expanded=True):
        ...
        with slider:
            st.session_state.slider_value = st.slider("Select number of iterations for Monte-Carlo simulation", 5000, 100000, value=st.session_state.slider_value, on_change=get_options_data(iterations=st.session_state.slider_value, risk_free=st.session_state.risk_free_rate/100))
        with risk_free:
            st.session_state.risk_free_rate = st.number_input("Risk free rate (%)", value=st.session_state.risk_free_rate, step=0.5, on_change=get_options_data(iterations=st.session_state.slider_value, risk_free=st.session_state.risk_free_rate/100))
